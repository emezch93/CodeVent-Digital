...
| `verify-certificate.html` | Public certificate authenticity checker — no login required |
| `admin-console/index.html` | Firebase-authenticated admin dashboard — support requests + certificate management |
...
## Certificate & Payment System (August 2026)

Course purchases, entitlements, and course-completion certificates are handled by a Cloudflare
Worker and a dedicated D1 database — separate from, and additive to, the pre-existing customer
support system that already lived on the same Worker.

### Architecture

```
Worker: codevent-support.emezch93.workers.dev
  ├── Support ticket system (unchanged)
  │     → D1: codevent-support   (table: requests)
  │
  └── Payment / entitlement / certificate system (new)
        → D1: codevent-central-db
              ├── customers
              ├── payments
              ├── products
              ├── entitlements
...
Both systems share one Worker (single JavaScript file, ES module format — required for D1
bindings) and reuse the same Firebase ID-token verification, `ADMIN_EMAILS` authorization check,
and Zoho Mail sending code already in place for support tickets. No second auth system, no
second email provider.

### Products

Four paid courses, each with a fixed price enforced server-side (never trusted from the browser):

| Product ID | Course | Price |
|---|---|---|
| `codevent-web-development-course` | Web Development | ₦15,000 |
| `codevent-nodejs-course` | Node.js | ₦10,000 |
| `codevent-machine-learning-course` | Machine Learning | ₦20,000 |
| `codevent-ai-automation` | AI Automation | ₦30,000 |

### Worker routes (added)

| Method | Path | Auth | Purpose |
|---|---|---|---|
| GET | `/verify-payment?reference=&product=` | none (Paystack-verified) | Verifies a Paystack transaction server-side, creates the customer/payment/entitlement rows, issues a signed access token |
| POST | `/verify-access` | none (token-verified) | Verifies a signed access token against a live entitlement |
| GET | `/verify-certificate?id=` | none | Public certificate authenticity check — powers `verify-certificate.html` |
| GET | `/api/certificates` | Firebase admin | List/search/filter certificates |
| GET | `/api/certificates/:id` | Firebase admin | Certificate detail (`:id` = internal row id) |
| GET | `/api/certificates/:id/download` | Firebase admin | Server-rendered printable certificate (HTML → browser print-to-PDF) |
| POST | `/api/certificates` | Firebase admin | Issue a certificate |
| POST | `/api/certificates/:id/resend` | Firebase admin | Resend the certificate email |
| POST | `/api/certificates/:id/revoke` | Firebase admin | Revoke a certificate (soft-delete; stays publicly checkable as "revoked") |

### Required Worker secrets

Set via the Cloudflare dashboard (Settings → Variables and Secrets, encrypted):

`PAYSTACK_SECRET_KEY`, `ACCESS_TOKEN_SECRET`, `CERTIFICATE_SECRET`, plus the pre-existing
`ADMIN_EMAILS`, `FIREBASE_PROJECT_ID`, and `ZOHO_*` secrets already used by the support system.

### Certificate lifecycle

A certificate is **not** issued automatically just because a payment exists. Payment creates an
*entitlement*; an admin manually confirms course completion and issues the certificate from the
Admin Console. Issuing without a matching entitlement on record is allowed (for students who paid
before this system existed) but is noted in `audit_logs` either way.

Certificate IDs look like `CV-WEB-2026-8F72KQ` — non-sequential, cryptographically random,
checked for uniqueness before insert. The certificate template embeds the existing brand asset
(`icons/icon-192x192.png`) by URL; no duplicate logo file was introduced.

### Admin Console

`admin-console/index.html` has a **Certificates** tab alongside the existing **Requests** tab —
same Firebase-authenticated session, same API client pattern, same visual language. From there an
admin can search/filter certificates, issue new ones (by internal customer ID — look it up in the
`codevent-central-db` → `customers` table by email if unknown), download, resend, or revoke.

### Public verification

`verify-certificate.html` — a standalone page, no login required — lets anyone enter a
certificate ID and see whether it's valid, revoked, or unknown. Linked from the footer of every
main page ("Verify Certificate") and shown on `shop.html`'s course cards.
...
- **Payments, entitlements, certificates:** Cloudflare Worker (`codevent-support`) + two D1 databases + Paystack + Zoho Mail — see [Certificate & Payment System](#certificate--payment-system-august-2026) below
...
├── privacy.html
├── terms.html
├── verify-certificate.html # public certificate authenticity checker
├── admin-console/          # Firebase-authenticated admin dashboard (requests + certificates)
│   ├── index.html
│   ├── admin.js
│   ├── admin.css
│   └── admin-config.js
